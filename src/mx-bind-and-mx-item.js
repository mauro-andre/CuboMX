...
import { parseAttrToBind, getComponentNameAttr, getProxyInfo, assignValue, createReaction, addReaction, parseAttrValue, twoWayBinding, } from "./hydration-helpers";
const resolveMXBind = (el, publicAPI) => {
    ...
    for (const attr of Array.from(el.attributes)) {
        ...
        if (!attrToBind || attrToBind.startsWith(":"))
            continue;
        const { proxy, componentName, componentAttr } = getProxyInfo(el, attr, publicAPI);
        if (!proxy) {
            if (componentName) {
                console.error(`[CuboMX] mx-bind directive failed: Component "${componentName}" not found`);
            }
            else {
                console.error(`[CuboMX] mx-bind directive failed: No mx-data component found in element or ancestors`);
            }
            continue;
        }
        ...
        twoWayBinding(attrToBind, componentAttr, proxy, el);
    }
};
const resolveMXItem = (el, publicAPI) => {
    el.__doNotProcessNode__ = true;
    const mainAttr = el.getAttributeNode("mx-item");
    if (!mainAttr)
        return;
    const { proxy, componentName, componentAttr } = getProxyInfo(el, mainAttr, publicAPI);
    if (!proxy) {
        if (componentName) {
            console.error(`[CuboMX] mx-item directive failed: Component "${componentName}" not found`);
        }
        else {
            console.error(`[CuboMX] mx-item directive failed: No mx-data component found in element or ancestors`);
        }
        return;
    }
    el.__itemProxy__ = createProxy({}, el);
