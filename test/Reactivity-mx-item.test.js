...
describe("mx-item Reactivity - Text Binding", () => {
    it("should update text in multiple elements when properties change", () => {
        ...
        expect(document.querySelector("#bio")?.textContent).toBe("Software Developer"); // Outros não mudam
...
describe("mx-item Reactivity - HTML Binding", () => {
    it("should update innerHTML when html property changes", () => {
        ...
        expect(document.querySelector("#content")?.innerHTML).toBe("<p>New <strong>bold</strong> content</p>");
        // Outros elementos não mudam
        expect(document.querySelector("#title")?.textContent).toBe("Post Title");
...
describe("mx-item Reactivity - Class Binding", () => {
    it("should update classes when property changes", () => {
        ...
        // Título não muda
        expect(document.querySelector("#title")?.textContent).toBe("Card Title");
    });
});
describe("mx-item Reactivity - Complex Scenarios", () => {
    it("should handle deeply nested elements", () => {
        ...
        expect(document.querySelector("#author")?.textContent).toBe("Jane Smith");
        expect(document.querySelector("#date")?.textContent).toBe("2024-12-31");
        expect(document.querySelector("#excerpt")?.textContent).toBe("New excerpt text");
        expect(document.querySelector("#link")?.getAttribute("href")).toBe("https://example.com/article");
    });
    it("should handle multiple properties on the same element", () => {
        ...
        expect(document.querySelector("#name")?.textContent).toBe("Updated Item");
    });
...
describe("mx-item Reactivity - Edge Cases", () => {
    it("should handle null and undefined values", () => {
        ...
        expect(document.querySelector("#text")?.textContent).toBe("Back to text");
    });
    ...
    it("should prevent direct assignment to ArrayItems", () => {
        ...
        CuboMX.list.items = [{ name: "New" }];
        expect(consoleErrorSpy).toHaveBeenCalledWith(expect.stringContaining('Cannot set property "items": it is an ArrayItems created by mx-item directive'));
        // The original array should not be replaced
        expect(CuboMX.list.items.length).toBe(1);
        expect(CuboMX.list.items[0].name).toBe("Item 1");
