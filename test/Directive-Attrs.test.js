...
describe("CuboMX - mx-bind Directive", () => {
    ...
    it("should trigger this.$watch for property changes", () => {
        ...
        expect(textWatcher).toHaveBeenCalledWith(
            "Texto Alterado",
            "Texto Inicial"
        );
    });
    ...
    it("should provide class manipulation helper methods", () => {
        CuboMX.component("myComp", { myAttrs: null });
        document.body.innerHTML = `
            <div mx-data="myComp">
        ...
        // 1. Test addClass
        attrs.addClass("added-class");
        expect(el.classList.contains("initial-class")).toBe(true);
        expect(el.classList.contains("added-class")).toBe(true);

        // 2. Test removeClass
        attrs.removeClass("initial-class");
        expect(el.classList.contains("initial-class")).toBe(false);
        expect(el.classList.contains("added-class")).toBe(true);

        // 3. Test toggleClass (turning it on)
        attrs.toggleClass("toggled-class");
        expect(el.classList.contains("toggled-class")).toBe(true);

        // 4. Test toggleClass (turning it off)
        attrs.toggleClass("toggled-class");
        expect(el.classList.contains("toggled-class")).toBe(false);
    });
});
