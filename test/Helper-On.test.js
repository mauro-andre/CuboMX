...
describe('CuboMX.on() Helper - JS Objects', () => {
    beforeEach(() => {
        CuboMX.reset();
    ...
    it('should attach an event listener and provide (el, event, item) to the callback', async () => {
        ...
        // Assert $item
        const expectedItem = CuboMX.listComp.items[1];
    ...
    it('should maintain the correct "this" context inside the callback', async () => {
        let contextInsideCallback = null;
        ...
        expect(contextInsideCallback).toBe(CuboMX.listComp);
        expect(contextInsideCallback.myProperty).toBe('hello world');
    });

    it('should handle the .prevent modifier', async () => {
        const clickSpy = vi.fn();
        CuboMX.component('linkComp', {
    ...
    it('should correctly provide el, event, and a defined item from an mx-item context', async () => {
        ...
        CuboMX.component('listComp', {
            ...
            init() {
                const listItems = this.$el.querySelectorAll('li');
                listItems.forEach(li => {
                    CuboMX.on(li, 'click', this.handleClick);
                });
            }
        });
